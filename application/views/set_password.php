      ...
      <div class="card-group">
        <div class="card bg-transparent2">
          <div class="card-block">
            <?php if ($status): ?>
              <div class="center">
                <h4 class="m-b-0"><span class="icon-text">Aktivasi Akun Berhasil</span></h4>
                <p class="text-muted">Silahkan Setting Password Anda</p>
              </div>
              <form action="<?=base_url()?>auth/save_password" method="post" id="form_password">
                <input type="hidden" name="user_id" value="<?=$user->id?>"/>
                <div class="form-group">
                  <select class="form-control" name="paket" id="paket">
                    <option value="">-- Pilih Paket Umroh --</option>
                    <?php foreach ($paket_umroh as $key): ?>
                    <option value="<?=$key->id_paket?>"><?=$key->nama_paket?> (Rp. <?=number_format($key->harga,0,',','.')?>)</option>
                    <?php endforeach; ?>
                  </select>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  <input type="text" class="form-control" name="presenter" id="presenter" placeholder="Kode Presenter (kosongkan jika tidak ada)">
                </div>
                <div class="form-group">
                ...
                <?php if (isset($referal)): ?>
                  <div class="form-group">
                    <p>Referal Anda : <?=$referal[0]->nama." (".$referal[0]->kode_user.")"?></p>
                  </div>
                <?php endif; ?>
                <div class="center">
